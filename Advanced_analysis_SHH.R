...
####Part I: More details about the top drivers####
    ...
    ms_tab_SHH<-ms_tab[which(ms_tab$'Z-statistics.SHH.vs.Others_DA_Isabel'>1.96 & ms_tab$'Z-statistics.SHH.Vs.others_DA_Xu'>1.96),]
    #ranked ms_tab_shh
    ms_tab_SHH<-ms_tab_SHH[order(ms_tab_SHH$'Z-statistics_combineDataDA_SHH_vs_others',decreasing=T),]
    out2excel(ms_tab_SHH,"/Volumes/XuYang/ATRT/analysis/ATRT_combined_Isabel_Xu/ATRT_combined_2020-09-22/DATA/shh_driver_table.xlsx")
    ms_tab_SHH<-ms_tab_SHH[!duplicated(ms_tab_SHH$gene_label),]
    sig_driver<-ms_tab_SHH[1:800,]
    #for sig_gene
    ms_tab_SHH_gene<-ms_tab[which(ms_tab$'Z-statistics_SHH_vs_others_DE_Isabel'>1.96 & ms_tab$'Z-statistics_SHH_vs_others_DE_Xu'>1.96),]
    ms_tab_SHH_gene<-ms_tab_SHH_gene[order(sig_gene$'Z-statistics_combinateDataDE_SHH_vs_others',decreasing=T),]
    ms_tab_SHH_gene<-ms_tab_SHH_gene[!duplicated(ms_tab_SHH_gene$gene_label),]
    sig_gene<-ms_tab_SHH_gene[1:800,]
    
    comp_name <- 'SHH.Vs.others' ## get the comparison name
    #1e-21 518
    sig_driver <- draw.volcanoPlot(dat=ms_tab_SHH,label_col='gene_label',logFC_col="logFC.SHH.Vs.others_DA_Xu",
                                   Pv_col="P.Value_combineDataDA_SHH_vs_others",logFC_thre=0,Pv_thre=1e-21,
    ...
    #1e-13
    sig_gene <- draw.volcanoPlot(dat=ms_tab_SHH,label_col='geneSymbol',logFC_col="logFC.combinateDataDE_SHH_vs_others",
                                 Pv_col="P.Value.combinateDataDE_SHH_vs_others",logFC_thre=1,Pv_thre=1e-3,
...
####QI.2: How to interpret the significance of top DA drivers ?####
    ...
    draw.GSEA.NetBID(DE=DE,profile_col='t',profile_trend='pos2neg',
                     ...
                     show_label=ms_tab_SHH[driver_list,'gene_label'],
                     driver_DA_Z=ms_tab_SHH[driver_list,"Z-statistics_combineDataDA_SHH_vs_others"],
                     driver_DE_Z=ms_tab_SHH[driver_list,"Z-statistics_combinateDataDE_SHH_vs_others"],
                     target_list=analysis.par$merge.network$target_list,
                     top_driver_number=30,target_nrow=2,target_col='RdBu',
                     left_annotation = 'high in SHH',right_annotation = 'high in others',
                     main=comp_name,target_col_type='PN',Z_sig_thre=1.64,profile_sig_thre = 1.64,
                     pdf_file=sprintf('%s/NetBID_GSEA_SHH.pdf',analysis.par$out.dir.PLOT))
...
####QI.3: What is the expression/activity pattern of these top DA drivers across sample subtypes?####    
    ...
    driver<-driver_list_full[1:50]
    draw.heatmap(mat=exp_mat2,use_genes=ms_tab_SHH[driver,'geneSymbol'],use_gene_label=ms_tab_SHH[driver,'gene_label'],
                 use_samples=colnames(ac_mat2),
...
####QI.4: What are the biological functions of these top DA drivers ?####
    ...
    out2excel(res_up,out.xlsx=sprintf('%s/fisher_res_SHH.xlsx',analysis.par$out.dir.PLOT))
    ...
    DE_res_up <- funcEnrich.Fisher(input_list=ms_tab_SHH[DE_list_up,'geneSymbol'],bg_list=unique(ms_tab[,'geneSymbol']),use_gs=c('H','CP:REACTOME','BP',"CP:KEGG","CGP"), Pv_thre=1,Pv_adj = 'none',min_gs_size = 30, max_gs_size = 500)
    DE_res_up_H <- funcEnrich.Fisher(input_list=ms_tab_SHH[DE_list_up,'geneSymbol'],bg_list=unique(ms_tab[,'geneSymbol']),use_gs='H', Pv_thre=1,Pv_adj = 'none',min_gs_size = 30, max_gs_size = 500)
    DE_res_up_kegg <- funcEnrich.Fisher(input_list=ms_tab_SHH[DE_list_up,'geneSymbol'],bg_list=unique(ms_tab[,'geneSymbol']),use_gs='CP:KEGG', Pv_thre=1,Pv_adj = 'none',min_gs_size = 30, max_gs_size = 500)
    ...
    draw.bubblePlot(driver_list= driver_list,show_label=ms_tab_SHH[driver_list_full,'gene_label'],
                    Z_val=ms_tab_SHH[driver_list_full,"Z-statistics_combineDataDA_SHH_vs_others"],
                    driver_type=NULL,
    ...
    ##Hallmark
    draw.bubblePlot(driver_list= driver_list,show_label=ms_tab_SHH[driver_list,'gene_label'],
                    Z_val=ms_tab_SHH[driver_list,"Z-statistics_combineDataDA_SHH_vs_others"],
                    driver_type=NULL,
                    target_list=analysis.par$merge.network$target_list,transfer2symbol2type=transfer_tab,
    ...
    G1  <- rownames(phe_info)[which(phe_info$`Subgroup_new`=='SHH')] # Experiment group
    G0  <- rownames(phe_info)[which(phe_info$`Subgroup_new`!='SHH')] # Combine other groups as the Control group
    
    DA_gs_bid_SHH <- getDE.BID.2G(eset=generate.eset(driver_ac_gs),G1=G1,G0=G0,G1_name='SHH',G0_name='others')
    DA_gs_bid_SHH_H<- getDE.BID.2G(eset=generate.eset(driver_ac_gs_H),G1=G1,G0=G0,G1_name='SHH',G0_name='others')
    DA_gs_bid_SHH_kegg<- getDE.BID.2G(eset=generate.eset(driver_ac_gs_kegg),G1=G1,G0=G0,G1_name='SHH',G0_name='others')
    
    DA_gs_bid_SHH_up<-DA_gs_bid_SHH[which(DA_gs_bid_SHH[,4]>0),]
    DA_gs_bid_SHH_down<-DA_gs_bid_SHH[which(DA_gs_bid_SHH[,4]<0),]
    
    out2excel(list(up=DA_gs_bid_SHH_up,down=DA_gs_bid_SHH_down),out.xlsx=sprintf('%s/gs_DA_SHH.xlsx',analysis.par$out.dir.PLOT))
    
    # Draw volcano plot for top significant gene sets
    DA_gs_SHH <- draw.volcanoPlot(dat= DA_gs_bid_SHH,label_col='ID',logFC_col='logFC',
                                  ...
                                  pdf_file=sprintf('%s/vocalno_GS_DA_SHH.pdf',analysis.par$out.dir.PLOT)) 
    
    
    DA_gs_SHH_H <- draw.volcanoPlot(dat= DA_gs_bid_SHH_H,label_col='ID',logFC_col='logFC',
                                    Pv_col='P.Value',logFC_thre=0,Pv_thre=1e-3,
                                    main='Volcano Plot for gene sets',show_label=TRUE,label_type = 'distribute',label_cex = 0.5,
                                    pdf_file=sprintf('%s/vocalno_GS_DA_SHH_H.pdf',analysis.par$out.dir.PLOT)) 
    DA_gs_SHH_kegg <- draw.volcanoPlot(dat= DA_gs_bid_SHH_kegg,label_col='ID',logFC_col='logFC',
                                       Pv_col='P.Value',logFC_thre=0,Pv_thre=1e-3,
                                       main='Volcano Plot for gene sets',show_label=TRUE,label_type = 'distribute',label_cex = 0.5,
                                       pdf_file=sprintf('%s/vocalno_GS_DA_SHH_kegg.pdf',analysis.par$out.dir.PLOT)) 
    DA_gs_SHH_up<-DA_gs_SHH[which(DA_gs_SHH[,2]>0),]
    DA_gs_SHH_down<-DA_gs_SHH[which(DA_gs_SHH[,2]<0),]
    DA_gs_SHH_H_up<-DA_gs_SHH_H[which(DA_gs_SHH_H[,2]>0),]
    DA_gs_SHH_kegg_up<-DA_gs_SHH_kegg[which(DA_gs_SHH_kegg[,2]>0),]
    ...
    draw.GSEA.NetBID.GS(DE=DA,name_col='ID',profile_col='logFC',profile_trend='pos2neg',
                        sig_gs_list = DA_gs_SHH_up$ID,
                        gs_DA_Z= DA_gs_bid_SHH[DA_gs_SHH_up$ID,'Z-statistics'],
    ...
    draw.GSEA.NetBID.GS(DE=DA,name_col='ID',profile_col='logFC',profile_trend='pos2neg',
                        sig_gs_list = DA_gs_SHH_H_up$ID,
                        gs_DA_Z= DA_gs_bid_SHH_H[DA_gs_SHH_H_up$ID,'Z-statistics'],
                        use_gs2gene = use_gs2gene,
                        top_gs_number=20,target_col='RdBu',
                        left_annotation = 'high in mutant',right_annotation = 'high in WT',
                        main= comp_name,Z_sig_thre=1.64,profile_sig_thre = 0,
    ...
    draw.GSEA.NetBID.GS(DE=DA,name_col='ID',profile_col='logFC',profile_trend='pos2neg',
                        sig_gs_list = DA_gs_SHH_kegg_up$ID,
                        gs_DA_Z= DA_gs_bid_SHH_kegg[DA_gs_SHH_kegg_up$ID,'Z-statistics'],
                        use_gs2gene = use_gs2gene,
                        top_gs_number=20,target_col='RdBu',
                        left_annotation = 'high in mutant',right_annotation = 'high in WT',
                        main= comp_name,Z_sig_thre=1.64,profile_sig_thre = 0,
